...
async function addRoom(socket) {
    socket.on("addRoom", async ({ hotelId }) => {
        try {
            const hotel = await Hotel.findById(hotelId);
            if (!hotel) {
                return socket.emit("error", { message: "Hotel not found" });
            }
            hotel.rooms += 1;
            await hotel.save();
...
module.exports = { add, getall, getById, getByname, update, deleteHotel, deleteAll,getMax , addRoom};